    ...
    def train_and_eval(self):
        ...
        def train(model, optimizer, criterion, stage=1):
            ...
            for i_batch, batch_data in enumerate(self.train_loader):
                ...
                self.losses.append([proc_loss / proc_size, nce_loss / proc_size, ba_loss / proc_size, task_loss / proc_size,infonce_loss / proc_size, cross_infonce_loss / proc_size,ap_loss_sum/proc_size])
        ...
        if not self.record:
            with open(str(self.hp.np_seed), 'w', newline='') as csvfile:
                writer = csv.writer(csvfile)
                # 写入字段名
                writer.writerow(["proc_loss","nce_loss","ba_loss" ,"task_loss","infonce_loss", "cross_infonce_loss","ap_loss"])
                # 写入数据行
